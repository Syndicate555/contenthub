...
async function testFeature6() {
  ...
  try {
    ...
    const badgesByType = allBadgesInDb.reduce((acc, b) => {
    ...
    console.log(`Progress: ${progress.earned}/${progress.total} (${progress.percentage}%)`);
    ...
    console.log(`  New total: ${xpResult.totalXp} XP (Level ${xpResult.level})`);
    if (xpResult.levelUp) {
      console.log(`  🎉 LEVEL UP! ${xpResult.previousLevel} → ${xpResult.level}`);
    }
    ...
    const byRarity = finalBadges.reduce((acc, b) => {
