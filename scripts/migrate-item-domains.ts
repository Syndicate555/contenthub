  ...
  try {
    ...
    for (const item of itemsWithoutDomains) {
      ...
      try {
        ...
        console.error(`  ✗ Failed to assign domain for item ${item.id}:`, error);
        skipped++;
      }
    }
    ...
    console.log(`Success rate: ${Math.round((assigned / itemsWithoutDomains.length) * 100)}%\n`);
    ...
    const unassigned = domainCounts.find(d => d.domainId === null)?._count || 0;
    if (unassigned > 0) {
